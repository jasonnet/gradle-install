...
# This is currently the latest version of Gradle.  If it's no longer available, you can find a new
# version here: https://services.gradle.org/distributions/
DEFAULTGRADLEVERSION=6.7

if [[ -n $1 ]] ; then
    MATCHLEN=`expr match "$1" '3[.][1-9][0-9]*'`
    HAYSTACKLEN=`expr length $1`
    #echo lengths are $MATCHLEN  $HAYSTACKLEN
    if [ "$MATCHLEN" = "$HAYSTACKLEN" ] ; then
        GRADLEVERSION=$1
    else
        echo " Error: the parameter must be a version number of the form n.n. For example:"
        echo "    6.7  "
        echo " Please try again with a version number of the right format or pass no parameter"
        echo " at all to install version $GRADLEVERSION."
        echo ""
        exit 15
    fi
else
...
DIRNAME2=gradle-${GRADLEVERSION}
FN2=gradle-${GRADLEVERSION}-bin.zip
...
if [ "$?" = "0" ] ; then
    ...
    URL2=https://services.gradle.org/distributions/${FN2}
    WGETPATH=`which wget`
    if [ "$?" = "0" ] ; then
        # wget is installed
        ...
        if [ "$?" = "0" ] ; then
            ...
            echo "   $URL2"
            echo " failed.  Please download it yourself from https://services.gradle.org/distributions/"
            echo " and try this script again.  If you find that version $GRADLEVERSION simply is no longer "
        ...
        if [ "$?" = "0" ] ; then
            ...
            echo " curl and wget are not found.  Please install them or download Gradle "
...
echo " Next this script will unpack the zip file in this directory"
...
if [ "${JAVA_HOME}F" = "F" ] ; then
    ...
    echo " which should allow you to maintain more than one instance of gradle."
    echo " "
...
FNTEMPLATE=setenv.gradle.template.source
PWD=`pwd`

GRADLEPATH=${PWD}/${DIRNAME2}/
echo export JAVA_HOME=${JAVA_HOME} >$FNTEMPLATE
echo export PATH=${JAVA_HOME}/bin:\${PATH} >>$FNTEMPLATE
echo export PATH=${GRADLEPATH}/bin:\${PATH} >>$FNTEMPLATE
echo " You will want to set your PATH to point to Gradle.  You can find a sample"
echo " source script here:  $FNTEMPLATE "
echo " You should copy it and edit it as appropriate.  You can invoke it via:"
echo "     source setenv.gradle.source"
echo " if you have named it  setenv.gradle.source . "
echo " "
echo " done"
